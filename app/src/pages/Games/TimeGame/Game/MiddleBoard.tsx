...
const MiddleBoard = ({cards}: {cards: cardType[]}) => {
  const { setNodeRef } = useDroppable({ id: BoardType.MIDDLE, data: { type: ItemType.BOARD } });
        ...
        <div className='w-full h-full p-4 flex flex-wrap justify-center gap-3'>
          {
            cards.length > 0 ?
              cards.map((item, index) => (
                <CardSortable card={item} key={`myboard_card_${index}`} board={BoardType.MIDDLE}/>
              ))
            :
